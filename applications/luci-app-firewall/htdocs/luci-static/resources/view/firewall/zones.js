	...
	renderZones: function(data) {
		...
		o.tooltip = function(section_id) {
			...
			if ((!family || family.indexOf('6') == -1) && (masq_src || masq_dest))
				return _('Limited ipv4 masquerading enabled');

			return null;
		};

		o = s.taboption('general', form.Flag, 'masq6', _('IPv6 Masquerading'),
			_('Enable network address and port translation IPv6 (NAT6 or NAPT6) for outbound traffic on this zone. This is typically enabled on the <em>wan</em> zone.'));
		o.editable = true;
		o.tooltip = function(section_id) {
			var family = uci.get('firewall', section_id, 'family')
			var masq_src = uci.get('firewall', section_id, 'masq_src')
			...
			if ((!family || family.indexOf('6') >=0) && (masq_src || masq_dest))
				return _('Limited ipv6 masquerading enabled');
