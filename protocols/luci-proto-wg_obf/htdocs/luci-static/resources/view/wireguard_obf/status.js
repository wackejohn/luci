...
function timestampToStr(timestamp) {
	if (timestamp < 1)
		return _('Never', 'No WireGuard Obfuscated peer handshake yet');
...
function handlePeerDetails(peer) {
	ui.showModal(_('Peer Details'), [
		ui.itemlist(E([]), [
			...
			_('Keep-Alive'), (peer.persistent_keepalive != 'off') ? _('every %ds', 'WireGuard Obfuscated keep alive interval').format(+peer.persistent_keepalive) : E('em', _('none')),
		]),
		E('div', { 'class': 'right' }, [
			E('button', {
				'class': 'btn cbi-button',
...
return view.extend({
	renderIfaces: function(ifaces) {
		var res = [
			E('h2', [ _('WireGuard Obfuscated Status') ])
		];

		for (var instanceName in ifaces) {
			res.push(
				E('h3', [ _('Instance "%h"', 'WireGuard Obfuscated instance heading').format(instanceName) ]),
				E('p', {
					'style': 'cursor:pointer',
					...
					E('span', { 'style': 'opacity:.8' }, [
						' · ',
						_('Port %d', 'WireGuard Obfuscated listen port').format(ifaces[instanceName].listen_port),
						' · ',
		...
		if (res.length == 1)
			res.push(E('p', { 'class': 'center', 'style': 'margin-top:5em' }, [
				E('em', [ _('No WireGuard Obfuscated interfaces configured.') ])
			]));
